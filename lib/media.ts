...
export async function getBaseMedia():Promise<MediaLibrary>{
  return JSON.parse(await readFile(path.join(process.cwd(),'data','media.json'),'utf8')) as MediaLibrary;
}
export async function getMedia():Promise<MediaLibrary>{
  const base=await getBaseMedia();
  const cms=await readCollection<any[]>('media',[]);
  const images=cms.filter(item=>item.status!=='Draft'&&item.type==='image').map(item=>({id:item.id,type:'image' as const,category:item.category||'general',section:item.category||'admin',title:item.title||'Uploaded image',alt:item.title||'AOLAN Machinery image',src:item.url||item.src,width:1200,height:900,bytes:item.bytes||0,source:'admin'}));
  const videos=cms.filter(item=>item.status!=='Draft'&&item.type==='video').map(item=>({id:item.id,type:'video' as const,category:item.category||'general',section:item.category||'admin',title:item.title||'Uploaded video',alt:item.title||'AOLAN Machinery video',src:item.url||item.src,poster:item.poster||'',width:1280,height:720,duration:0,bytes:item.bytes||0,source:'admin'}));
