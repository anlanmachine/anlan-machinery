...
export default async function Detail({params}:{params:Promise<{slug:string}>}){
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/><div className="pt-28">
    ...
    <section className="mx-auto grid max-w-7xl gap-12 px-5 pb-20 lg:grid-cols-2">
      ...
      <div><p className="eyebrow">{product.brand} - {product.subCategory.replaceAll('-',' ')}</p><h1 className="mt-3 text-4xl font-black tracking-tight sm:text-6xl">{product.model}</h1><p className="mt-6 text-lg leading-8 text-gray-600">{description}</p>{specifications.length>0&&<div className="mt-8 grid grid-cols-2 gap-px overflow-hidden rounded-2xl bg-gray-200">{specifications.map(([key,value])=><div className="bg-white p-4" key={key}><p className="text-xs text-gray-400">{key}</p><b>{value}</b></div>)}</div>}<div className="mt-8 flex flex-wrap gap-3"><Link href={`/products/${category.path}`} className="inline-flex items-center gap-2 rounded-full border border-black/15 px-6 py-3 font-black"><ArrowLeft size={18}/>Back to {category.label}</Link><a href={wa(`Hi, I want a quotation for ${product.name}. Please send price, specs, and shipping details.`)} target="_blank" rel="noopener" className="inline-flex items-center gap-2 rounded-full bg-lime px-6 py-3 font-black"><MessageCircle size={18}/>Request Quote</a><a href={COMPANY_EMAIL_LINK} className="inline-flex items-center gap-2 rounded-full border border-black/15 px-6 py-3 font-black"><Mail size={18}/>Email: {COMPANY_EMAIL}</a></div></div>
    </section>
  </div></>;
}
