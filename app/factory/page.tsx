...
export default async function FactoryPage(){
  ...
  const cms=(await readCollection<any[]>('factory',[])).filter(item=>item.status!=='Draft');
  const cmsImages=cms.flatMap(item=>(item.images||[]).map((src:string,index:number)=>({id:`${item.id}-${index}`,type:'image' as const,category:'factory',section:item.category||'factory',title:item.title,alt:item.title,src,width:1200,height:900,bytes:0,source:'admin'})));
  const cmsVideos=cms.filter(item=>item.video).map(item=>({id:`${item.id}-video`,type:'video' as const,category:'factory',section:item.category||'factory',title:item.title||'Factory video',alt:item.title||'AOLAN Machinery factory video',src:String(item.video),poster:String(item.images?.[0]||''),width:1280,height:720,duration:0,bytes:0,source:'admin'}));
  const images=[...cmsImages,...media.images.filter(item=>item.category==='factory')];
  const videos=[...cmsVideos,...media.videos.filter(item=>item.category==='factory')];
