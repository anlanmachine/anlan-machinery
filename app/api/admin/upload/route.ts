...
export async function POST(request:NextRequest){
  try{
    ...
    for(const file of files){
      ...
      const asset={id:`media-${Date.now()}-${uploaded.length}`,title:file.name,type:file.type.startsWith('image/')?'image':file.type==='video/mp4'?'video':'document',url:saved.url,src:saved.url,bytes:file.size,mime:file.type,category:folder,status:'Published',createdAt:nowIso(),updatedAt:nowIso()};
      assets.unshift(asset);
      uploaded.push(asset);
    }
    await writeCollection('media',assets);
    return NextResponse.json({success:true,assets,images:uploaded.filter(item=>item.type==='image').map(item=>item.url)});
