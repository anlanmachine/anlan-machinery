...
import {productDescription,productSlug,productSpecifications,type CatalogProduct} from '@/lib/catalog';
...
export function ProductListSection({products,emptyMessage}:{products:CatalogProduct[];emptyMessage?:string}){
  ...
  return <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{products.map(product=>{
    ...
    return <article key={product.id} className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-black/5">
      <Link href={`/products/${productSlug(product.model)}`} className="block">
      ...
      <div className="p-6">
        ...
        <h2 className="mt-2 text-2xl font-black"><Link href={`/products/${productSlug(product.model)}`}>{product.model}</Link></h2>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          <Link href={`/products/${productSlug(product.model)}`} className="rounded-full border border-black/15 px-5 py-3 font-bold">View Details</Link>
